...
class CreateWorkoutViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBAction func createWorkout(_ sender: Any) {
        let duration = Int(sliderDuracion.value*100)
        let date = fechaPicker.date
        let type = WorkoutType.wtype[workoutPicker.selectedRow(inComponent: 0)].rawValue
        ...
        //let regreso = self.storyboard?.instantiateViewController(withIdentifier: "WorkoutsViewController") as! WorkoutsViewController
